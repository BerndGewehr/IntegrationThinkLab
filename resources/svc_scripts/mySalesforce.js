...
function __buildCrateOppyScreen(myObjectStr) {
    ...
    __img.src = "/files/customizer/salesforce/salesforce_big.png?repoName=think-samples";
    __img.width = 200;
    __img.height = 100;
...
function _captureEvent (target) {
    if (target.id.startsWith('ic-tile:urn:lsid:lconn.ibm.com:activitystreams.story:')) {
        ...
        myChild.firstChild.addEventListener('click', function (e) {
            ...
            xmlHttpReq.open('GET', 'https://apps.na.collabserv.com/connections/opensocial/basic/rest/activitystreams/@me/@all/@all/' + _story, true);
            xmlHttpReq.setRequestHeader('Content-Type', 'application/json');
            xmlHttpReq.onreadystatechange = function() {
                if (xmlHttpReq.readyState == 4) {
                    if ((xmlHttpReq.status == 200)) {
